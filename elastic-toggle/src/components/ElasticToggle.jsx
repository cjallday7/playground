...
function ElasticToggle({ isOn, onToggle }) {
  const [internalIsOn, setInternalIsOn] = useState(false)
  
  // Support both controlled and uncontrolled usage
  const controlled = isOn !== undefined
  const on = controlled ? isOn : internalIsOn
  const setOn = controlled ? onToggle : setInternalIsOn
  ...
  const handlePointerUp = () => {
    ...
    if ((on && currentX < threshold) || (!on && currentX > threshold)) {
      setOn(!on)
    }
  ...
  return (
    ...
    >
      <svg width="80" height="40" viewBox="0 0 80 40">
        <rect x="0" y="0" width="80" height="40" rx="20" fill={on ? '#4ade80' : '#444'} />
        <motion.ellipse
